...
class InAppNotificationService {
  ...
  Future<String> createNotification(InAppNotification notification) async {
    try {
      debugPrint('[InAppNotifService] Creating notification for userId: ${notification.userId}');
      debugPrint('[InAppNotifService] Notification data: title="${notification.title}", category="${notification.category}"');
      
      final docRef = await _firestore.collection(_collectionPath).add(
            notification.toMap(),
          );
      
      debugPrint('[InAppNotifService] ✅ Notification created in Firestore with ID: ${docRef.id}');
      return docRef.id;
    } catch (e) {
      debugPrint('[InAppNotifService] ❌ Error creating in-app notification: $e');
      debugPrint('[InAppNotifService] Error type: ${e.runtimeType}');
